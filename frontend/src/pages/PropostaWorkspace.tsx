...
import { novaCotacaoComPropostaPath } from '@/lib/cotacaoFornecedores';
...
export default function PropostaWorkspace({ proposta = null, onClose }: { proposta?: Proposta | null; onClose?: () => void }) {
  ...
  const [propostaTab, setPropostaTab] = useState<string>('dados');
  const numericClass =
  ...
  const load = useCallback(() => {
    // Workspace dedicado nÃ£o recarrega listagem. Mantido para nÃ£o quebrar chamadas internas.
  }, []);
  ...
  const refreshItensAposHomologacao = useCallback(async () => {
    ...
    try {
      ...
      /* mantÃ©m estado local se falhar refresh */
    }
  }, [editing?.id]);
  ...
  const clienteResumoLabel = useMemo(() => {
    if (!clienteAvulso && selectedCliente && selectedCliente.id === form.cliente_id) {
      return selectedCliente.razao_social || selectedCliente.nome_fantasia || `Cliente #${form.cliente_id}`;
    }
    if (!clienteAvulso && form.cliente_id) {
      return `Cliente #${form.cliente_id}`;
    }
    if (clienteAvulso && form.cliente_avulso_nome?.trim()) {
      return form.cliente_avulso_nome.trim();
    }
    if (clienteAvulso) {
      return 'Cliente avulso — nome pendente';
    }
    return 'Cliente não informado';
  }, [clienteAvulso, selectedCliente, form.cliente_id, form.cliente_avulso_nome]);

  const clienteResumoBadge = useMemo(() => {
    if (clienteAvulso) return { label: 'Avulso', cls: 'erp-badge-warning' } as const;
    if (form.cliente_id) return { label: 'Cadastrado', cls: 'erp-badge-success' } as const;
    return null;
  }, [clienteAvulso, form.cliente_id]);

  const statusResumo = useMemo(() => statusPropostaUi(editing ?? ({ ...form, id: editing?.id ?? 0, itens } as unknown as Proposta)), [editing, form, itens]);

  const resumoStatusValidadeText = useMemo(() => {
    if (validadeCalculadaIso) {
      const diasRest = diasValidadeEntreDatas(dataHojeIso(), validadeCalculadaIso);
      const validadeFmt = formatDateBr(validadeCalculadaIso);
      if (typeof diasRest === 'number' && diasRest >= 0) {
        return `${statusResumo.label} · Válida até ${validadeFmt} (${diasRest} dias)`;
      }
      return `${statusResumo.label} · Válida até ${validadeFmt}`;
    }
    return statusResumo.label;
  }, [statusResumo, validadeCalculadaIso]);
    ...
    if (
      ...
      void (async () => {
        ...
        try {
          ...
          // Sem bloqueio na fase 1: mantÃ©m fluxo comercial atual.
        }
      })();
    }
  };
...
return (
    <div className="px-4 pb-5 sm:px-5 sm:pb-6">
      <PageHeader
        title={editing ? 'Proposta ' + editing.numero : 'Nova Proposta'}
        description="Dados da proposta comercial"
        actions={
          <button type="button" className="erp-btn-outline shrink-0" onClick={closeForm}>
            Voltar para listagem
          </button>
        }
      />
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-x-3.5 gap-y-1.5 rounded-lg border border-border bg-muted/15 px-4 py-2 text-sm sm:grid-cols-2 sm:px-5 sm:py-2 lg:grid-cols-4 lg:gap-x-3 lg:gap-y-1.5">
          <div>
            <span className="text-[11px] uppercase tracking-wide text-muted-foreground/80 block">Proposta</span>
            <span className="font-semibold text-foreground truncate">{editing?.numero?.trim() || form.numero?.trim() || 'Nova Proposta'}</span>
            <span className="text-[11px] uppercase tracking-wide text-muted-foreground/80 block mt-1">Data</span>
            <span className="text-sm text-foreground">{form.data ? formatDateBr(form.data) : 'Data pendente'}</span>
          </div>
          <div>
            <span className="text-[11px] uppercase tracking-wide text-muted-foreground/80 block">Cliente</span>
            <span className="font-medium truncate block" title={clienteResumoLabel}>{clienteResumoLabel}</span>
          </div>
          <div>
            <span className="text-[11px] uppercase tracking-wide text-muted-foreground/80 block">Status / Validade</span>
            <div className="flex items-center gap-2 mt-0.5">
              <StatusBadge status={statusResumo.label} tone={statusResumo.tone === 'default' ? undefined : (statusResumo.tone as any)} />
              <span className="text-xs text-muted-foreground">{resumoStatusValidadeText}</span>
            </div>
          </div>
          <div>
            <span className="text-[11px] uppercase tracking-wide text-muted-foreground/80 block">Total</span>
            <span className="font-semibold tabular-nums text-foreground">{formatMoneyBr(total)}</span>
          </div>
        </div>

        <Tabs value={propostaTab} onValueChange={setPropostaTab} className="flex flex-col min-h-0">
          <TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto whitespace-nowrap bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/90 border border-border border-b-0 rounded-t-lg px-3 pt-2.5 pb-1 sm:px-4">
            ...
            <TabsTrigger value="pedido-venda">Pedido & Venda</TabsTrigger>
          </TabsList>
        ...
        <div className="space-y-4 mb-4">
          <div className="space-y-6">
            <h3 className="text-sm font-semibold text-foreground border-b border-border pb-2">A. Dados comerciais</h3>
            <div className="space-y-4">
          ...
          <div className="space-y-4 pt-4 border-t border-border">
            <h3 className="text-sm font-semibold text-foreground border-b border-border pb-2">B. Condições comerciais</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2 border-t border-border pt-4">
                <h3 className="text-sm font-semibold text-foreground border-b border-border pb-2 mb-4">C. Comunicação / Observações</h3>
          ...
          <div className="pt-4 border-t border-border">
            <h3 className="text-sm font-semibold text-foreground border-b border-border pb-2 mb-4">D. Contexto fiscal da proposta</h3>
          ...
          <TabsContent value="itens" className="mt-0 space-y-4">
            <ComercialModalSection title="Itens" className="mb-4">
              ...
              <div key={item.id} className="mb-4 rounded-md border border-border p-3 space-y-4">
                ...
                {itemAvulso && !ncmFiscalDigitsValid(item.ncm_avulso || '') && (
                    <div className="mb-2 rounded-md border border-amber-500/40 bg-amber-500/10 px-3 py-1.5 text-xs text-amber-950 dark:text-amber-100">
                      Item avulso sem NCM válido (8 dígitos): informe o NCM para buscar a Regra Fiscal ou vincule um produto cadastrado.
                    </div>
                  )}
                ...
                <details className="rounded-md border border-border bg-muted/10 group">
                  ...
                  <div className="p-3 pt-0 space-y-3">
                    <div className="space-y-2">
                      <p className="text-[11px] font-medium text-muted-foreground">Preço base e aplicado</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-2 items-end">
                        <div>
                          <label className="text-xs text-muted-foreground">Preço base (custo ÷ 0,60)</label>
                          <div className="erp-input h-8 text-sm flex items-center">{formatMoneyBr(item.preco_sugerido)}</div>
                        </div>
                        <div>
                          <label className="text-xs text-muted-foreground">Preço aplicado</label>
                          <div className="erp-input h-8 text-sm flex items-center font-medium">{formatMoneyBr(item.preco_final)}</div>
                        </div>
                        <div>
                        ...
                        <div>
                          <label className="text-xs text-muted-foreground">Desconto (R$)</label>
                          <div className="erp-input h-8 text-sm flex items-center">{formatMoneyBr(item.desconto)}</div>
                        </div>
                        <div>
                          <label className="text-xs text-muted-foreground">Receita após desconto</label>
                          <div className="erp-input h-8 text-sm flex items-center font-medium">{formatMoneyBr(computeReceitaComercialItem(item))}</div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <details className="rounded-md border border-border bg-muted/10 group">
                  ...
                  <div className="p-3 pt-0 space-y-3">
                    <div className="space-y-2">
                      <p className="text-[11px] font-medium text-muted-foreground">Custos e cargas totais do item</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-2 items-end">
                        <div>
                          <label className="text-xs text-muted-foreground">Custo carregado total</label>
                          <div className="erp-input h-8 text-sm flex items-center">{formatMoneyBr(toNumber(item.quantidade_negociada ?? item.quantidade) * toNumber(item.custo_final))}</div>
                        </div>
                        <div>
                          <label className="text-xs text-muted-foreground">Cargas/despesas estimadas</label>
                          <div className="erp-input h-8 text-sm flex items-center">
                            {formatMoneyBr(
                              toNumber(item.quantidade_negociada ?? item.quantidade) *
                              (valCarga + toNumber(item.frete_saida) + toNumber(item.outras_despesas_saida))
                            )}
                          </div>
                        </div>
                        <div>
                    ...
                    <div className="space-y-2">
                      <p className="text-[11px] font-medium text-muted-foreground">Resultado estimado</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-2 items-end border-t border-border/60 pt-2">
                        <div>
                          <label className="text-xs text-muted-foreground">Custo carregado total</label>
                          <div className="erp-input h-8 text-sm flex items-center">{formatMoneyBr(toNumber(item.quantidade_negociada ?? item.quantidade) * toNumber(item.custo_final))}</div>
                        </div>
                        ...
                        <div>
                          <label className="text-xs text-muted-foreground">Resultado estimado</label>
                          <div className="erp-input h-8 text-sm flex items-center font-medium">{formatMoneyBr(computeResultadoEstimadoItem(item))}</div>
                        </div>
                        <div>
                          <label className="text-xs text-muted-foreground">Margem contrib. estimada</label>
                          <div className="erp-input h-8 text-sm flex items-center">{formatPercent(computeMargemContribuicaoEstimadaItem(item))}</div>
                        </div>
                ...
                {/* 5. Contexto fiscal considerado */}
                <details className="rounded-md border border-border bg-muted/10 group">
                  <summary className="cursor-pointer px-3 py-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground select-none">
                    Contexto fiscal considerado
                  </summary>
                ...
                <details className="rounded-md border border-border bg-muted/10 group">
                  ...
                  <div className="p-3 pt-0 space-y-3">
                    <div className="space-y-2">
                      <p className="text-[11px] font-medium text-muted-foreground">Resultado do item (legado)</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-2 items-end">
                        <div>
                          <label className="text-xs text-muted-foreground">Preço base (custo ÷ 0,60)</label>
            ...
            {itens.length === 0 ? (
              <div className="rounded-md border border-dashed border-border bg-muted/10 px-4 py-5 text-center">
                <p className="text-sm font-medium text-foreground">Nenhum item adicionado à proposta.</p>
                <p className="mt-1 text-xs text-muted-foreground">Adicione o primeiro item para iniciar a formação comercial.</p>
              </div>
            ) : null}
            {itens.length > 0 ? (
            ...
            {itens.length > 0 ? <ComercialModalSection title="Totais" className="mb-4">
              ...
              <div className="mt-4 rounded-md border border-primary/30 bg-primary/5 p-3">
                <p className="text-xs font-medium uppercase tracking-wide text-primary">Rentabilidade estimada (gerencial — não inclui frete de cabeçalho)</p>
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="rounded-md border border-border bg-background/80 p-3">
                    <p className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Receita comercial consolidada</p>
                    <p className="mt-1 text-lg font-semibold tabular-nums">{formatMoneyBr(computeReceitaComercialConsolidada(itens))}</p>
                  </div>
                  <div className="rounded-md border border-border bg-background/80 p-3">
                    <p className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Resultado estimado consolidado</p>
                    <p className="mt-1 text-lg font-semibold tabular-nums">{formatMoneyBr(computeResultadoEstimadoConsolidado(itens))}</p>
                  </div>
                  <div className="rounded-md border border-border bg-background/80 p-3">
                    <p className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Margem contrib. estimada</p>
                    <p className="mt-1 text-lg font-semibold tabular-nums">{formatPercent(computeMargemContribuicaoEstimadaConsolidada(itens))}</p>
                  </div>
                </div>
              </div>
            </ComercialModalSection> : null}
          </TabsContent>

<TabsContent value="pedido-venda" className="mt-0 space-y-3">
            {!editing?.id ? (
              <div className="rounded-md border border-border bg-muted/10 px-4 py-4 text-center">
                <p className="text-sm font-medium text-foreground">Pedido & Venda</p>
                <p className="mt-2 text-xs text-muted-foreground">
                  Salve a proposta primeiro para habilitar: geração de Pedido de Venda, recuperação, histórico comercial, análise financeira e cotação com fornecedores.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
          <div className="mt-2 rounded-md border border-border bg-muted/30 p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <button type="button" className="erp-btn-outline erp-btn-sm" onClick={() => navigate(novaCotacaoComPropostaPath(editing.id))}>
      ...
      <div className="rounded-lg border border-border bg-card px-4 py-3 shadow-sm sm:px-4 sm:py-3.5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-8">
            <div>
              <span className="text-[11px] uppercase tracking-wide text-muted-foreground/80 block">Contexto</span>
              <span className="font-medium text-sm">{editing?.numero?.trim() || form.numero?.trim() || 'Nova Proposta'}</span>
              <span className="text-xs text-muted-foreground block">{statusResumo.label}</span>
            </div>
            <div>
              <span className="text-[11px] uppercase tracking-wide text-muted-foreground/80 block">Resumo comercial</span>
          ...
          <div className="flex w-full flex-col-reverse gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:justify-end">
            <div className="mr-2 text-left sm:text-right">
              <span className="text-[11px] uppercase tracking-wide text-muted-foreground/80 block">Total</span>
              <span className="font-bold tabular-nums text-2xl tracking-tight">{formatMoneyBr(total)}</span>
            </div>
            ...
            {editing?.id ? (
              ...
              <span className="text-xs text-muted-foreground self-center">{MSG_SALVE_ANTES_PDF}</span>
            )}
