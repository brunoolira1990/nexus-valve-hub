...
} from '@/services/api/nfeEntradaHistoricaImportada';
import type {
...
const ensureCorridaAdicional = (raw: unknown, ordem = 1): ItemCertificadoFornecedorCorrida => {
  ...
  return {
    ...
    composicao_json: (obj.composicao_json as Record<string, string>) || {},
    ensaio_tracao_json: (obj.ensaio_tracao_json as Record<string, string>) || {},
    ensaio_impacto_json: (obj.ensaio_impacto_json as Record<string, string>) || {},
...
const mergeCorridasAdicionais = (
  ...
  for (const nova of novas) {
    ...
    usados.add(k);
    resultado.push({ ...nova, id: nova.id });
  }

  return resultado.map((c, idx) => ({ ...c, ordem: idx + 1 }));
};
...
export function CertificadoFornecedorWorkspace({ certificadoId, onSaved, onCancel }: Props) {
  ...
  // Carregar certificado quando certificadoId muda
  useEffect(() => {
    if (!certificadoId) {
      setEditing(null);
      setForm(emptyForm());
    ...
    certificadosFornecedorService.getById(certificadoId)
      .then((row: CertificadoFornecedorEntrada) => {
        setEditing(row);
        setForm({
          ...row,
      ...
      .catch((e) => setSaveErrors(formatApiErrors(e)));
  }, [certificadoId]);

  // Busca de NF-e operacional (debounce)
  useEffect(() => {
    let cancelled = false;
    const t = window.setTimeout(() => {
      ...
      nfeEntradasService
        ...
        .then((rows) => { if (!cancelled) setNfEntradas(rows); })
        .catch(() => { if (!cancelled) setNfEntradas([]); });
    }, 350);
    return () => { cancelled = true; window.clearTimeout(t); };
  }, [buscaNfOperacional]);

  // Busca de NF-e histórica (debounce)
  useEffect(() => {
    let cancelled = false;
    const t = window.setTimeout(() => {
      ...
      nfeEntradaHistoricaImportadaService
        ...
        .then((rows) => { if (!cancelled) setNfHistoricas(rows); })
        .catch(() => { if (!cancelled) setNfHistoricas([]); });
    }, 350);
    return () => { cancelled = true; window.clearTimeout(t); };
  }, [buscaNfHistorica]);
  ...
  const aplicarNumeroCabecalhoEmTodosOsItens = () => {
    const numeroCabecalhoAtual = (form.numero_certificado_fornecedor || '').trim();
    if (!numeroCabecalhoAtual) {
    ...
    setForm((p) => ({
      ...
      itens: p.itens.map((it) => ({
        ...it,
        numero_certificado_fornecedor_item: numeroCabecalhoAtual,
      })),
    }));
