...
export default function RelatorioContasReceberPage() {
  const fetcher = useCallback(
    (q: Record<string, string>) => financeiroService.relatorioContasReceber(q),
    [],
  );
  ...
  const cards: RelatorioCardItem[] = data
    ? [
        { titulo: 'Total em aberto', metrica: data.cards.total_aberto, tone: 'receber' },
        { titulo: 'Total vencido', metrica: data.cards.total_vencido, tone: 'alerta' },
        {
          titulo: 'Recebido no período',
          metrica: data.cards.recebido_periodo,
          tone: 'neutro',
        },
        { titulo: 'Parcialmente recebido', metrica: data.cards.total_parcial, tone: 'neutro' },
        {
          titulo: 'Quantidade de títulos',
          metrica: { valor: '0', quantidade: data.cards.quantidade_titulos },
  ...
  const abrirBaixa = async (ln: RelatorioLinhaTitulo) => {
    const titulo = await financeiroService.getContaReceber(ln.id);
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Títulos a receber, totais por vencimento e recebimentos no período selecionado."
        actions={
          <div className="flex flex-wrap gap-2 items-center">
            <RelatorioPdfActions endpoint="contas-receber" filtros={filtros} className="flex gap-2 mb-0" />
      ...
      {!loading && !error && data ? (
        <>
          ...
          {data.linhas.length > 0 ? (
            ...
            <EmptyState message={EMPTY_STATE_RELATORIO['contas-receber']} />
          )}
        </>
      ) : null}
