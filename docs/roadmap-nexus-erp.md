...
- **ERP 4.0.15.0 Parte 2A (Entrada Própria 1 — fundação backend e numeração):** `tipo_origem=ENTRADA_PROPRIA_EMITIDA`; status operacionais ampliados; campos fiscais/SEFAZ nullable em `NFeEntrada` e itens (`ncm`, `cfop`, `unidade`, `impostos_json`); `NFeNumeracaoConfiguracao.tipo_operacao` (`saida` | `entrada_propria`, default `saida`); `obter_config_numeracao_entrada()` + `reservar_numeracao_nfe_entrada()`; migration `0042_nfentrada_entrada_propria_emitida_4015`; testes `test_nfe_entrada_4015_emissao_homologacao.py`; **sem builder XML tpNF=0, assinatura A1, transmissão SEFAZ, DANFE, endpoints de emissão, estoque, CP ou apuração**
- **ERP 4.0.15.0 Parte 2B (Entrada Própria 1 — XML oficial tpNF=0, preview e validação):** `dados_preview.py`, `xml_oficial.py`, `validar_pre_emissao_homologacao_entrada()`; endpoints `validar-emissao-homologacao`, `preview-xml-oficial`, `gerar-xml-oficial-emissao`, `xml-gerado`, `reservar-numeracao`; campo `nat_op`; migration `0043_nfeentrada_nat_op_4015`; testes `test_nfe_entrada_4015_xml_preview.py`; **sem assinatura A1, transmissão SEFAZ, DANFE, endpoint emitir-homologacao, produção, estoque, CP ou apuração**
- **ERP 4.0.15.x Fase 3C (NF-e Saída produção SEFAZ — UI segura, flag desligada):** `permissoes_producao.py`, `conferencia_producao.py`; campos `producao_habilitada`, `pode_emitir_producao`, `emissao_producao`; `NFeSaidaEmissaoProducaoPanel.tsx` (badge vermelho, modal confirmação dupla, `GET validar-emissao-producao`); permissão explícita (`admin`/`administrador`/`fiscal_nfe_producao` — grupo `fiscal` genérico excluído); testes `test_nfe_saida_producao_4015_conferencia_ui.py` (6), `nfeSaidaProducaoUi4015.test.tsx` (15); **sem ligar flag, sem transmissão real, homologação 402 intacta**
...
## ERP 4.0.15.0.1 — Revisão leve (Manifestação do Destinatário)

Revisão de integração sem execução de testes automatizados:

- Permissões `admin` / `administrador` / `fiscal` também em listagem e detalhe da API.
- UI: status finais de manifestação bloqueiam nova manifestação; fechamento mensal usa mês corrente por padrão.
...
Primeira versão do fluxo de **Manifestação do Destinatário** para NF-e de entrada:

- Tela **Fiscal > Manifestação Destinatário** (`/manifestacao-destinatario`).
- API `GET/POST /api/fiscal/manifestacao-destinatario/` — listagem, detalhe, consulta manual DF-e, manifestação por documento, download XML, preview de fechamento mensal.
- Modelos `NFeDestinadaManifestacao` e `NFeDestinadaManifestacaoEvento` (migration `0049`).
- Consulta manual via certificado A1 da empresa (produção); resumos `resNFe` persistidos.
- Manifestação manual auditável: Ciência, Confirmação, Desconhecimento, Operação não Realizada (com justificativa).
- Download XML explícito → classificação **Base DF-e Importada** (`NFeEntradaHistoricaImportada`), sem financeiro/estoque/apuração automática.
- **CT-e / transportadora:** fase complementar **4.0.15.1** (fora do escopo principal desta entrega).
- **Não substitui** conferência fiscal humana.
- Certificados, senhas e secrets **nunca** devem ser versionados (.env, .pfx, .pem).

## ERP 4.1 (futuro) — Fechamento fiscal/contábil e DF-e (legado)
...
- Central de DF-e; ~~manifestação do destinatário~~ (**iniciada em ERP 4.0.15**); consulta/download de XMLs.
