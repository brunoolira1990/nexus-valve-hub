...
from apps.fiscal.danfe_render import RENDERER_BFR, RENDERER_HTML, render_danfe_conferencia_pdf
...
class DanfeBrazilFiscalReportPocTests(TestCase):
    ...
    def test_sanitiza_xml_previa_comentarios(self):
        ...
        limpo = sanitizar_xml_para_bfr(xml, ajustes_poc=True, nfe_saida_id=1)
        self.assertNotIn('<!--', limpo)
        pdf = gerar_danfe_bfr_de_xml_string(limpo)
        self.assertTrue(pdf.startswith(b'%PDF'))

    @mock.patch('apps.fiscal.nfe_integracao.danfe_brazil_fiscal_report.gerar_danfe_bfr_de_nfe_saida_preview')
    def test_falha_bfr_nao_quebra_endpoint_html(self, mock_bfr):
        ...
        with override_settings(FISCAL_DANFE_RENDERER=RENDERER_BFR):
    ...
    @override_settings(FISCAL_DANFE_RENDERER=RENDERER_HTML)
    def test_renderer_html_default(self):
        ...
        self.assertTrue(pdf.startswith(b'%PDF'))
        self.assertIn(meta.get('render_engine', ''), ('weasyprint_html', 'reportlab_canvas_fallback'))

    @override_settings(FISCAL_DANFE_RENDERER=RENDERER_BFR)
    def test_renderer_bfr_quando_ativado(self):
        from apps.fiscal.danfe_conferencia import montar_dados_danfe_conferencia
    ...
    @override_settings(FISCAL_DANFE_RENDERER='reportlab_fallback')
    def test_renderer_reportlab_fallback(self):
        from apps.fiscal.danfe_conferencia import montar_dados_danfe_conferencia
        ...
        pdf, meta = render_danfe_conferencia_pdf(dados, nfe_saida_id=nf.pk)
        self.assertTrue(pdf.startswith(b'%PDF'))
        self.assertEqual(meta.get('render_engine'), 'reportlab_canvas_fallback')
