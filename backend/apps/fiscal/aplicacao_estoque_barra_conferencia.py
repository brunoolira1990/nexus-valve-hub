...
def gerar_codigo_interno_barra(
    ...
    return f'EB-{item_conferencia_id}-{ordem:03d}-{equivalencia_id}'
...
def aplicar_estoque_barras_composicao_conferencia(
    ...
    *,
    ...
    """
    Cria ou reutiliza EstoqueBarra para cada linha de composição validada.
    Idempotente por equivalencia_entrada (OneToOne).
    """
    ...
    equiv_ids_ativos: set[int] = set()

    for equiv in equivs:
        metros = _dec(equiv.metros)
        if metros <= 0:
            continue
        equiv_ids_ativos.add(equiv.id)
        ...
        codigo = gerar_codigo_interno_barra(item.id, equiv.id, equiv.ordem)
    ...
    for barra in item.estoque_barras.select_for_update().exclude(
        equivalencia_entrada_id__in=equiv_ids_ativos,
    ).exclude(status=EstoqueBarra.Status.CANCELADA):
        _cancelar_barra(barra)
