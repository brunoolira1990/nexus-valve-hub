...
const emptyEmpresa: Omit<Empresa, 'id'> = {
  ...
  telefone:'', email:'', site:'', empresa_pai_id:null, certificado_digital:'', senha_certificado:''
};
...
const Empresas = () => {
  ...
  const openNew = () => { setEditing(null); setForm({...emptyEmpresa}); setTab('dados'); setModalOpen(true); };
  const openEdit = (e: Empresa) => { setEditing(e); setForm(e); setTab('dados'); setModalOpen(true); };
  const handleDelete = async (id: number) => { if (confirm('Excluir?')) { await empresasService.delete(id); load(); } };

  const handleSave = async () => {
    if (editing) await empresasService.update(editing.id, form);
    else await empresasService.create(form);
  ...
  return (
    <div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar Empresa' : 'Nova Empresa'} size="lg">
        ...
        {tab === 'certificado' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="erp-label">Arquivo do Certificado</label><input type="file" className="erp-input mt-1" /></div>
