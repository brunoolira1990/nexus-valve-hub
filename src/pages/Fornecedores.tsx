...
const Fornecedores = () => {
  ...
  const openEdit = (e: Fornecedor) => { setEditing(e); setForm(e); setModalOpen(true); };
  const handleDelete = async (id: number) => { if (confirm('Excluir?')) { await fornecedoresService.delete(id); load(); } };
  const handleSave = async () => {
    if (editing) await fornecedoresService.update(editing.id, form);
    else await fornecedoresService.create(form);
  ...
  return (
    <div>
      <PageHeader title="Fornecedores" onAdd={openNew} addLabel="Novo Fornecedor" searchValue={search} onSearch={setSearch} />
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar Fornecedor' : 'Novo Fornecedor'} size="lg">
